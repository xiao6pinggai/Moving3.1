...
def main(opt):
    ...
    if False:
        ...
        DataVal = torch.utils.data.Subset(_DataValRaw, indices=range(min(len(_DataValRaw), 5)))
...
#     #####################################################################
    ...
    # ========== 双阶段训练配置（核心修改） ==========
    STAGE1_EPOCHS = opt.stage1_epochs  # 第一阶段仅训练I2PNet的epoch数
    if opt.two_stages:
        # 初始冻结除I2PNet外的所有参数（阶段1）
        for name, param in model.named_parameters():
            if "I2PNet" not in name:
                param.requires_grad = False
        print("===== 双阶段训练：阶段1（Epoch 0~9）仅训练I2PNet =====")
    else:
        # 非双阶段：所有参数可训练，保持原逻辑
        for param in model.parameters():
            param.requires_grad = True
    # ========== 优化器配置（兼容双阶段/非双阶段） ==========
    # 定义参数组：区分I2PNet和其他参数（用于双阶段阶段2的学习率微调）
    param_groups = [
        {"params": [p for name, p in model.named_parameters() if "I2PNet" in name and p.requires_grad], 
         "lr": opt.lr_net1, "name": "I2PNet"},
        {"params": [p for name, p in model.named_parameters() if "I2PNet" not in name and p.requires_grad], 
         "lr": opt.lr, "name": "others"}
    ]
    # 过滤空参数组（阶段1时"others"参数组为空）
    param_groups = [g for g in param_groups if len(g["params"]) > 0]
    
    # 初始化优化器（保持原优化器类型，如Adam）
    ...
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        ...
        if opt.two_stages and epoch == STAGE1_EPOCHS+1:
            # 解冻所有参数
            for param in model.parameters():
                param.requires_grad = True
            # 补充“others”参数到优化器（保留原I2PNet的优化器状态）
            others_params = [p for name, p in model.named_parameters() if "I2PNet" not in name]
            optimizer.add_param_group({
                "params": others_params,
                "lr": opt.lr,  # 其他网络用初始lr
                "name": "others"
            })
            print(f"===== 双阶段训练：阶段2（Epoch ≥{STAGE1_EPOCHS+1}）联合训练所有参数 =====")
        ...
        if epoch in opt.lr_step:
            ...
            print('Drop LR to', param_group['lr'] * (0.1 ** (opt.lr_step.index(epoch) + 1)))
    logger.close()
