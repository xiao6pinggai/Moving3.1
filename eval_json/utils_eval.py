...
class eval_metric():
    ...
    def get_result(self, img_size=[1024,1024], seq_len=325):
        ...
        fa_1 = self.fp/(img_size[0]*img_size[1]*seq_len)
    ...
    def update(self, gt, det, return_match=False):
        # return_match=True 时额外返回本帧通过阈值的匹配对 (N,2)，每行 [det_idx, gt_idx]，
        # 供上层按 gt 轨迹类别归拢 tp/fn/fp。计数逻辑与不传该参数时完全一致。
            ...
            if (det.shape[0] > 0):
                ...
                if min(cost_matrix.shape) > 0:
                    ...
                    if self.eval_mode == 'iou':
                        matched_indices = self.linear_assignment(-cost_matrix)
                        matched_matrix = cost_matrix[matched_indices[:, 0], matched_indices[:, 1]]
                        keep = matched_matrix > self.iou_th
                        matched_results = matched_matrix[keep]
                    elif self.eval_mode == 'dis':
                        matched_indices = self.linear_assignment(cost_matrix)
                        matched_matrix = cost_matrix[matched_indices[:, 0], matched_indices[:, 1]]
                        keep = matched_matrix < self.dis_th
                        matched_results = matched_matrix[keep]
                    else:
                        raise Exception('Not a valid eval mode!!!!')
                    if return_match:
                        matched_pairs = matched_indices[keep]
                else:
