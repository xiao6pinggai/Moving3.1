...
class COCO_mir(data.Dataset):
    opt = opts().parse()
    num_classes = 1
    default_resolution = [1080,1920]
    mean = np.array([0.41397, 0.41397, 0.41397],
                    dtype=np.float32).reshape(1, 1, 3)
    std = np.array([0.19892, 0.19892, 0.19892],
                   dtype=np.float32).reshape(1, 1, 3)
    def __init__(self, opt, split):
        super(COCO_mir, self).__init__()
        ...
        self.split = split
        if split == 'train':
            self.resolution = [256, 256] # 裁剪 # 352 416
            # self.resolution = [512, 512]
            self.resolution_ori = [352, 416] # 先行后列
            self.annot_path = os.path.join(
                self.img_dir0, 'annotations',
                'train_mot_new.json')
        else:
            self.resolution = [352, 416]
            self.annot_path = os.path.join(
                self.img_dir0, 'annotations',
                'test_mot_new.json')
    ...
    def run_eval_just(self, save_dir, time_str):
        coco_dets = self.coco.loadRes('{}/results_{}.json'.format(save_dir, time_str))
        coco_eval = COCOeval(self.coco, coco_dets, "bbox")
        coco_eval.evaluate()
        coco_eval.accumulate()
