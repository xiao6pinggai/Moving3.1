...
class COCO_sdm_car(data.Dataset):
    # opt = opts().parse()
    num_classes = 1
    default_resolution = [1080,1920]
    mean = np.array([0.30695, 0.30695, 0.30695],
                    dtype=np.float32).reshape(1, 1, 3)
    std = np.array([0.21774, 0.21774, 0.21774],
                   dtype=np.float32).reshape(1, 1, 3)
    def __init__(self, opt, split):
        super(COCO_sdm_car, self).__init__()
        ...
        if split == 'train':
            ...
            self.resolution_ori = [1088, 1920] # 先行后列
