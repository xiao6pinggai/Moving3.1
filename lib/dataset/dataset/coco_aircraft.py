...
class COCO_aircraft(data.Dataset):
    ...
    def __init__(self, opt, split):
        super(COCO_aircraft, self).__init__()
        ...
        if split == 'train':
            ...
            self.resolution = [256, 256] # lhg
            self.resolution_ori = [512, 512]
            self.annot_path = os.path.join(
                self.opt.data_dir, 'annotations',
                'train_mot_new.json').format(split)
        else:
            self.resolution = [512, 512]
            self.annot_path = os.path.join(
                self.opt.data_dir, 'annotations',
                'test_mot_new.json').format(split)
        ...
        for image_id in all_bboxes:
            for cls_ind in all_bboxes[image_id]:
                ...
                for bbox in all_bboxes[image_id][cls_ind]:
                    ...
                    score = bbox[4]
                    if score <= 0 or bbox[2] <= bbox[0] or bbox[3] <= bbox[1]:
                        continue
                    bbox[2] -= bbox[0]
                    bbox[3] -= bbox[1]
