...
def test_update(opt, split, modelPath, show_flag, results_name, save_mat=False, epoch=0):
    ...
    conf_filtered = opt.conf_filtered # 0.2
    ...
    # 日志1：打印数据文件夹数量和名称
    print(f"【DEBUG】有效数据文件夹数：{len(data_folder_list)}，前5列表为：{data_folder_list[:5]}")
    ...
    for ii in range(len(data_folder_list)):
        ...
        img_list = [i for i in img_list if i.endswith(('.png','.jpg'))]
        img_list.sort()
        imgs_number = len(img_list)
        if imgs_number == 0:
            print("图片数量为0，无法更新伪标签！")
        overlap_flag = 0
        ...
        for ii_m in range(len(img_list)):
            ...
            txt_path = os.path.join(txt_coords_ori_folder, os.path.splitext(img_list[ii_m])[0] + '.txt')
        ...
        for pk in range(patch_num):
            ...
            patch_xml_path = [
                                    f.replace("img1", opt.xmlname).rsplit('.', 1)[0] + ".xml" 
                                    for f in patch_ims_path
                                ] # rsplit('.', 1) 表示从右边开始，以 . 为分隔符，只切分 1 次。
            batch_dict, meta, patch_imgs, input_imgs = preprocess(patch_ims_path, DataVal, patch_xml_path)
            for k in input_imgs:
                if k == 'batch_size':
                    continue
                input_imgs[k] = torch.from_numpy(input_imgs[k]).to(opt.device)
            output, dets = process(model, input_imgs, return_time, opt, opt.K) # dets为返回的框
            # 后处理
            rets, dets_post = post_process(dets, meta, num_classes, max_per_image=max_per_image) # 转换框格式，并进行下采样补偿
            # 后处理
            count = -1
            for ret in rets:
                count=count+1
        ...
        for traj_i in trajs:
            ...
            v_mean = abs(v).mean()
            # print(v_mean)
            if v_mean < 0.55: # 过滤移动速度低于0.55像素的轨迹
                continue
            trajs_filt.append(traj_i) # 得到模型预测结果通过sort滤波后的轨迹
        ...
        for kk in range(len(det_for_images)):
            ...
            txt_update_save_name = os.path.join(save_txt_update_folder, os.path.splitext(img_list[kk])[0] + '.txt')
