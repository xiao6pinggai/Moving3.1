...
class TripletMotionConsistencyMotionPairSparseConv(nn.Module):
    """V29: V23 main path with sigmoid trajectory aggregation.

    Compared with V23, this version keeps the core prev-cur-next motion pair
    attention unchanged, but removes disabled/fallback branches: global
    alignment, self-attention, non-marginal value aggregation, feature-key
    fallback, Python top-k fallback, and legacy state-dict filling.
    """
    ...
    def __init__(
        ...
        self.topk = int(topk)
        if self.topk <= 0:
            raise ValueError("cosv29 requires tmc_topk > 0")
        ...
        self.gd_conv = (
            GroupedDilatedSparseConv(
                self.in_channels,
                self.GDilation,
                indice_key=f"cosv29_gd_c{self.in_channels}",
            )
            if self.GD
            else None
        )
    ...
    def _get_index_map(self, x_conv, indices, index_map=None):
        ...
        if index_map is not None:
            if not index_map.is_cuda:
                raise RuntimeError("cosv29 requires CUDA index_map; no Python fallback is available")
    ...
    def _check_conv_contract(input_features, x_conv, in_channels):
        if x_conv.features.shape != input_features.shape or x_conv.features.shape[1] != in_channels:
            raise RuntimeError(
                "cosv29 requires conv to preserve sparse point count, order, and channels; "
    ...
    def _select_topk_by_feature_distance(self, indices, features_norm, index_map):
        if not indices.is_cuda or not features_norm.is_cuda or not index_map.is_cuda:
            raise RuntimeError("cosv29 Top-K requires CUDA tensors; no Python fallback is available")
    ...
    def forward(self, x, index_map=None):
        ...
        if not input_features.is_cuda:
            raise RuntimeError("cosv29 requires CUDA sparse features; no Python fallback is available")
        ...
        if not indices.is_cuda or not features.is_cuda:
            raise RuntimeError("cosv29 requires CUDA indices and features; no Python fallback is available")
...
TripletMotionConsistencyMotionPairSparseConvV29 = TripletMotionConsistencyMotionPairSparseConv
