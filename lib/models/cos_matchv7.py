...
class SparseSymmetricCosineAttention(nn.Module):
    def __init__(self, in_channels, kernel_size=3, alpha=1.0, stride=1, 
                 ...
                 use_maxpool=False, temporal_dilation=2): # [修改1] 新增 dilation 参数，默认 2
        super().__init__()
        self.k = kernel_size
        ...
        self.temporal_dilation = temporal_dilation # 记录 dilation
    ...
    def forward(self, x):
        ...
        # [修改2] 引入 temporal_dilation
        # 我们只拉长时间间隔，不拉长空间偏移(off_y)。
        # 这样对于慢速目标，我们在 t-2 帧查看同样的 3x3 区域，更容易发现它是否偏离了中心。
        ...
        if self.use_biqkv:
            ...
            traj_consistency = (k_prev * k_next).sum(dim=-1) * self.scale_factor
        ...
        # [修改3] 边界判断需要考虑 dilation
        # 如果是 dilation=2, 那么 t=0 和 t=1 的帧都无法向左找邻居
        ...
        # 计算中心点的索引 (假设 kernel_size=3, K2=9, 则 center=4)
        center_idx = K2 // 2
        
        # 创建掩码：找出所有最大索引指向中心点的样本
        # max_indices shape: (N, 1)
        is_center_mask = (max_indices == center_idx)
        
        # 将这些样本的 max_scores 置为 0
        # 物理意义：如果最相似的是我自己(中心)，则权重为0，即不进行任何增强，保留原始特征
        ...
        if self.use_qkv or self.use_biqkv:
            ...
            # [重要 Bug 修复]
            # 你之前的代码是: features + (features * max_scores * self.alpha)
            # 这意味着你计算了半天的 injected_v 根本没用上！
            # 应该改为使用 injected_v:
            enhanced_features = features + (injected_v * max_scores * self.alpha)
...
if __name__ == "__main__":
    ...
    model = SparseSymmetricCosineAttention(in_channels=3, kernel_size=9, use_qkv=False, use_maxpool=False, alpha=1, use_biqkv=False, temporal_dilation=1).to(device)
