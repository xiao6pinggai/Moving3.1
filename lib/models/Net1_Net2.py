...
class Net1Net2(nn.Module):
    ...
    def forward_patch(self, batch, max_patch_w):
        ...
        def run_patch(patch_batch, patch_w):
            ...
            z['sampling_rate'] = torch.ones((), device=patch_batch['input'].device)
            return z
        ...
        for start in range(0, w, max_patch_w):
            ...
            with torch.no_grad():
                z_patch = run_patch(patch_batch, end - start)

            if z_merged is None:
                z_merged = {}
                for head in self.heads:
                    out_shape = list(z_patch[head].shape)
                    out_shape[-1] = w
                    z_merged[head] = z_patch[head].new_empty(out_shape)

                out_shape = list(z_patch['hm_large_heatmap'].shape)
                out_shape[-1] = w
                z_merged['hm_large_heatmap'] = z_patch['hm_large_heatmap'].new_empty(out_shape)

                out_shape = list(z_patch['soft_mask'].shape)
                out_shape[-1] = w
                z_merged['soft_mask'] = z_patch['soft_mask'].new_empty(out_shape)
                z_merged['sampling_rate'] = torch.ones((), device=batch['input'].device)

            for head in self.heads:
                z_merged[head][..., start:end].copy_(z_patch[head])
            z_merged['hm_large_heatmap'][..., start:end].copy_(z_patch['hm_large_heatmap'])
            z_merged['soft_mask'][..., start:end].copy_(z_patch['soft_mask'])

            del z_patch, patch_batch
