...
class SparseSymmetricCosineAttention(nn.Module):
    def __init__(self, in_channels, kernel_size=5, alpha=0.5, stride=1, 
        ...
        if self.exclude_center:
            ...
            valid_offsets_y = offsets_y[valid_offsets]
            valid_offsets_x = offsets_x[valid_offsets]
        else:
            valid_offsets_y = offsets_y
            valid_offsets_x = offsets_x
        spatial_offsets = (valid_offsets_y * self.scale_y + valid_offsets_x)
        prev_spatial_offset_indices = ((-valid_offsets_y + self.r) * self.k + (-valid_offsets_x + self.r)).long()
        next_spatial_offset_indices = ((valid_offsets_y + self.r) * self.k + (valid_offsets_x + self.r)).long()
        self.register_buffer('spatial_offsets', spatial_offsets)
        self.register_buffer('prev_spatial_offset_indices', prev_spatial_offset_indices)
        self.register_buffer('next_spatial_offset_indices', next_spatial_offset_indices)
        self.offset_chunk_size = 64
    ...
    def forward(self, x):
        ...
        # 4. Sparse neighbor search via pre-computed offsets.
        # Build the key window once, but compute cosine only for real sparse hits.
        ...
        s_prev = features.new_zeros((N, K2))
        s_next = features.new_zeros((N, K2))
        center_rows = torch.arange(N, device=features.device).unsqueeze(1).expand(N, K2).reshape(-1)

        if mask_prev_flat.any():
            valid_prev = mask_prev_flat.nonzero(as_tuple=False).squeeze(1)
            prev_scores = (
                features_norm[center_rows[valid_prev]]
                * sorted_features[ptr_prev[valid_prev]]
            ).sum(dim=1).clamp_min_(0)
            s_prev.reshape(-1)[valid_prev] = prev_scores

        if mask_next_flat.any():
            valid_next = mask_next_flat.nonzero(as_tuple=False).squeeze(1)
            next_scores = (
                features_norm[center_rows[valid_next]]
                * sorted_features[ptr_next[valid_next]]
            ).sum(dim=1).clamp_min_(0)
            s_next.reshape(-1)[valid_next] = next_scores

        # 5. Distance-aware symmetric fusion.
        #    Previous-frame hits are scattered by their true relative coordinate
        #    (-dy, -dx), then mirrored to compare with next-frame hits in the
        #    same motion direction coordinate system.
        s_prev_map = features.new_zeros((N, self.k * self.k))
        s_next_map = features.new_zeros((N, self.k * self.k))
        s_prev_map[:, self.prev_spatial_offset_indices] = s_prev
        s_next_map[:, self.next_spatial_offset_indices] = s_next

        s_prev_map = s_prev_map.view(N, self.k, self.k)
        s_next_map = s_next_map.view(N, self.k, self.k)
        s_prev_mirror = torch.flip(s_prev_map, dims=[1, 2])

        prev_flat = s_prev_mirror.view(N, -1)
        next_flat = s_next_map.view(N, -1)

        prev_max_scores, prev_max_idx = prev_flat.max(dim=1, keepdim=True)
        next_max_scores, next_max_idx = next_flat.max(dim=1, keepdim=True)

        prev_max_y = (prev_max_idx // self.k).float()
        prev_max_x = (prev_max_idx % self.k).float()
        next_max_y = (next_max_idx // self.k).float()
        next_max_x = (next_max_idx % self.k).float()

        peak_distance = torch.sqrt(
            (prev_max_y - next_max_y).pow(2) +
            (prev_max_x - next_max_x).pow(2)
        )
        distance_scale = max(1.0, self.r * 0.5)
        distance_gamma = 4.0
        eps = 1e-5
        distance_scores = 1.0 / (1.0 + (peak_distance / (distance_scale + eps)).pow(distance_gamma))
        both_sides_valid = ((prev_max_scores > 0) & (next_max_scores > 0)).float()
        ...
        # Mid-frame score is jointly controlled by the two peak similarities
        # and their mirrored-position distance. Boundary frames keep the
        # original single-side fallback.
        max_scores = (
            (prev_max_scores * next_max_scores * distance_scores * both_sides_valid * is_mid)
            + (next_max_scores * next_max_scores * is_start)
            + (prev_max_scores * prev_max_scores * is_end)
        )

        # 6. Adaptive Intensity Inverse Gain
        # 修改时间：2026-05-24
