...
class TZSLinear(nn.Module):
    """Temporal zero-sum linear mixing for per-frame foreground emphasis."""
    ...
    def __init__(
        ...
        if self.kernel_size[1:] != (1, 1):
            raise ValueError("TZSLinear only supports spatial kernel size 1x1")
        if self.stride != (1, 1, 1):
            raise ValueError("TZSLinear only supports stride=1")
        if self.padding != (0, 0, 0):
            raise ValueError("TZSLinear only supports padding=0")
        if self.dilation != (1, 1, 1):
            raise ValueError("TZSLinear only supports dilation=1")
        if self.kernel_size[0] <= 1:
            raise ValueError("TZSLinear requires at least 2 frames for temporal zero-sum mixing")
        ...
        self.weight = nn.Parameter(
            torch.empty(out_channels, self.in_channels_per_group, temporal_size, temporal_size)
        )
        if bias:
        ...
        init_weight = torch.full(
            (out_channels, self.in_channels_per_group, temporal_size, temporal_size),
            -1.0 / (temporal_size - 1),
        )
        diag_index = torch.arange(temporal_size)
        init_weight[:, :, diag_index, diag_index] = 1.0
        with torch.no_grad():
    ...
    def forward(self, x):
        if x.dim() != 5:
            raise ValueError(f"TZSLinear expects a 5D tensor, got shape {tuple(x.shape)}")

        n, _, t, h, w = x.shape
        temporal_size = self.kernel_size[0]
        if t != temporal_size:
            raise ValueError(
                f"TZSLinear expects {temporal_size} frames, but received {t}"
            )

        centered_weights = self.weight - self.weight.mean(dim=3, keepdim=True)
        x_grouped = x.view(n, self.groups, self.in_channels_per_group, t, h, w)
        grouped_weights = centered_weights.view(
            self.groups,
