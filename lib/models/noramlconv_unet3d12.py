...
class UNet3DWithATDCDilation2(Module):
    def __init__(self, num_channels=3, num_classes=1, feat_channels=[32,64,128,256], residual=None, 
        ...
        # 编码器：卷积块
        self.enc_conv1 = Conv3D_Block(num_channels, feat_channels[0], kernel=3, stride=1, padding=1,residual=residual)
        self.enc_conv2 = Conv3D_Block_with_ATDC(feat_channels[0], feat_channels[1], kernel=3, stride=1, padding=1,residual=residual)
        self.enc_conv3 = Conv3D_Block(feat_channels[1], feat_channels[2], kernel=3, stride=1, padding=1,residual=residual)
        ...
        if len(self.feat_channels) >= 4:
            self.enc_conv4 = Conv3D_Block(feat_channels[2], feat_channels[3], kernel=3, stride=1, padding=1,residual=residual) # 瓶颈层
        ...
        # 解码器：卷积块
        if len(self.feat_channels) >= 4:
            self.dec_conv4 = Conv3D_Block(2 * feat_channels[3], feat_channels[3], kernel=3, stride=1, padding=1,residual=residual)
        
        self.dec_conv3 = Conv3D_Block(2 * feat_channels[2], feat_channels[2], kernel=3, stride=1, padding=1,residual=residual)
        self.dec_conv2 = Conv3D_Block(2 * feat_channels[1], feat_channels[1], kernel=3, stride=1, padding=1,residual=residual)
        self.dec_conv1 = Conv3D_Block(2 * feat_channels[0], feat_channels[0], kernel=3, stride=1, padding=1,residual=residual)
