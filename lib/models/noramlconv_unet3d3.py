...
class UNet3DATDCDilation(Module):
    def __init__(self, num_channels=3, num_classes=1, feat_channels=[32,64,128,256], residual=None, 
        ...
        # 编码器：卷积块
        self.enc_conv1 = Conv3D_Block_ATDC_Dilation(num_channels, feat_channels[0], residual=residual, groups=groups)
        self.enc_conv2 = Conv3D_Block_ATDC_Dilation(feat_channels[0], feat_channels[1], residual=residual, groups=groups)
        self.enc_conv3 = Conv3D_Block_ATDC_Dilation(feat_channels[1], feat_channels[2], residual=residual, groups=groups)
        
        if len(self.feat_channels) >= 4:
            self.enc_conv4 = Conv3D_Block_ATDC_Dilation(feat_channels[2], feat_channels[3], residual=residual, groups=groups) # 瓶颈层
