...
class UNet3DCatATDC(Module):
    def __init__(self, num_channels=3, num_classes=1, feat_channels=[32,64,128,256], residual=None, 
        ...
        self.atdc1 = MultiBranchATDCWithBN(num_channels, feat_channels[0], kernel_size=3, dilations=[3, 5], stride=1, padding=0, bias=False)
        self.atdc2 = MultiBranchATDCWithBN(feat_channels[0], feat_channels[1], kernel_size=3, dilations=[3, 5], stride=1, padding=0, bias=False)
        self.atdc3 = MultiBranchATDCWithBN(feat_channels[1], feat_channels[2], kernel_size=3, dilations=[3, 5], stride=1, padding=0, bias=False)
        self.fusion1 = Conv3d(2*feat_channels[0], feat_channels[0], kernel_size=1, stride=1, padding=0, bias=False)
        self.fusion2 = Conv3d(2*feat_channels[1], feat_channels[1], kernel_size=1, stride=1, padding=0, bias=False)
        self.fusion3 = Conv3d(2*feat_channels[2], feat_channels[2], kernel_size=1, stride=1, padding=0, bias=False)
        if len(self.feat_channels) >= 4:
            ...
            self.atdc4 = MultiBranchATDCWithBN(feat_channels[2], feat_channels[3], kernel_size=3, dilations=[3, 5], stride=1, padding=0, bias=False)
            self.fusion4 = Conv3d(2*feat_channels[3], feat_channels[3], kernel_size=1, stride=1, padding=0, bias=False)
        # 解码器：卷积块
        if len(self.feat_channels) >= 4:
    ...
    def forward(self, x):
        ...
        if len(self.feat_channels) - 1 == 3:
            ...
            enc1 = self.fusion1(torch.cat([self.enc_conv1(x),self.atdc1(x)],dim=1))
            # enc1 = self.fusion1(self.enc_conv1(x),self.atdc1(x))
            down1 = self.down1(enc1)                # Downsample

            # enc2 = self.enc_conv2(down1)            # [B, C1, D/s, H/2, W/2]
            enc2 = self.fusion2(torch.cat([self.enc_conv2(down1),self.atdc2(down1)],dim=1))
            # del down1
            down2 = self.down2(enc2)                # Downsample

            # enc3 = self.enc_conv3(down2)            # [B, C2, D/s2, H/4, W/4]
            enc3 = self.fusion3(torch.cat([self.enc_conv3(down2),self.atdc3(down2)],dim=1))
            # del down2
