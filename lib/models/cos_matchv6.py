...
class SparseSymmetricCosineAttention(nn.Module):
    def __init__(self, in_channels, kernel_size=3, alpha=1.0, stride=1, 
        ...
        if self.use_qkv or self.use_biqkv:
            ...
            # [修改点 1]: Q 和 K 分离为两个独立的投影层
            # Query 投影
            self.proj_q = spconv.SubMConv3d(
                in_channels, in_channels, kernel_size=1, bias=False, indice_key=indice_key
            )
            # Key 投影
            self.proj_k = spconv.SubMConv3d(
                in_channels, in_channels, kernel_size=1, bias=False, indice_key=indice_key
            )
    ...
    def forward(self, x):
        ...
        if self.use_qkv or self.use_biqkv:
            ...
            # [修改点 2]: 独立计算 Q 和 K 的特征
            q_feat = self.proj_q(x_norm).features  # Query 特征
            k_feat = self.proj_k(x_norm).features  # Key 特征
            v_feat = self.proj_v(x_norm).features  # Value 特征
            
            # [关键]: K 和 V 需要根据哈希排序 (sorted_keys) 来重排，以便根据 ptr_prev/next 查找邻居
            sorted_k = k_feat[sort_idx]
            sorted_v = v_feat[sort_idx]
            
            # Gather Neighbors (使用 sorted_k)
            ...
            # Current Query (使用原始顺序的 q_feat)
            # 这里的 N 对应当前处理的每个点，不需要 sort
            q_curr = q_feat.view(N, 1, C)

        # === [改进版] Bi-QKV: 轨迹门控注意力 ===
        if self.use_biqkv:
            # 1. 标准 Attention 分数 (Query-Key Matching)
            # 计算 Q(当前) 与 K(邻居) 的非对称匹配
            ...
            # 2. 轨迹一致性分数
            # 这里的 k_prev 和 k_next 都是经过 proj_k 映射后的特征
            # 它们在同一特征空间内，可以进行点积比较
