...
class UNet3DwithZZLB(Module):
    def __init__(self, num_channels=3, num_classes=1, feat_channels=[32,64,128,256], residual=None, 
        ...
        # 编码器：卷积块
        self.enc_conv1 = Conv3D_Block(num_channels, feat_channels[0], kernel=3, stride=1, padding=1,residual=residual, groups=1)
    ...
    def forward(self, batch):
        ...
        img = batch['input']
        img_gray = batch['input_gray']
        x = self.preprocess(img_gray,img)
        # ---------------------------------------------------
...
class UNet3DATDC(Module):
    def __init__(self, num_channels=3, num_classes=1, feat_channels=[32,64,128,256], residual=None, 
        ...
        # 编码器：卷积块
        self.enc_conv1 = Conv3D_Block_ATDC(num_channels, feat_channels[0], residual=residual, groups=groups)
        self.enc_conv2 = Conv3D_Block_ATDC(feat_channels[0], feat_channels[1], residual=residual, groups=groups)
        self.enc_conv3 = Conv3D_Block_ATDC(feat_channels[1], feat_channels[2], residual=residual, groups=groups)
        
        if len(self.feat_channels) >= 4:
            self.enc_conv4 = Conv3D_Block_ATDC(feat_channels[2], feat_channels[3], residual=residual, groups=groups) # 瓶颈层
