...
class GroupTemporalAtrousPyramid3(nn.Module):
    ...
    def forward(self, x): # 不做0和
        ...
        out1 = self.forward_normal_conv(x_splits[1], self.branch1, dilation_t=self.d_zero1)
        out1 = self.bn1(out1)
        out1 = self.leakyrelu(out1) # 注意：如果是差分特征，LeakyReLU 其实比 ReLU 更好
        
        # [Branch 2] Zero-sum (Motion Medium)
        out2 = self.forward_normal_conv(x_splits[2], self.branch2, dilation_t=self.d_zero2)
        out2 = self.bn2(out2)
        out2 = self.leakyrelu(out2)
        
        # [Branch 3] Zero-sum (Motion Large)
        out3 = self.forward_normal_conv(x_splits[3], self.branch3, dilation_t=self.d_zero3)
        out3 = self.bn3(out3)
...
class Conv3D_Block_Group_Dilation(Module):
    """3D 卷积块"""
    def __init__(self, in_feat, out_feat, kernel=3, stride=1, padding=1, residual=None, groups=1):
        super().__init__()
        self.conv1 = Sequential(
        ...
        self.DepthwiseSeparableConv3d1 = Sequential(
            BasicConv3d(in_feat, out_feat, kernel_size=(1,kernel,kernel), stride=(1,stride,stride), padding=(0,padding,padding), bias=False, groups=groups),
            # BasicConv3d(out_feat, out_feat, kernel_size=(1,kernel,1), stride=(1,stride,1), padding=(0,padding,0), bias=False, groups=groups),
            GroupTemporalAtrousPyramid3(out_feat, out_feat, kernel_size=(kernel,1,1), stride=(stride,1,1), padding=(padding,0,0), bias=False),
        )
        self.DepthwiseSeparableConv3d2 = Sequential(
            BasicConv3d(out_feat, out_feat, kernel_size=(1,kernel,kernel), stride=(1,stride,stride), padding=(0,padding,padding), bias=False, groups=groups),
            # BasicConv3d(out_feat, out_feat, kernel_size=(1,kernel,1), stride=(1,stride,1), padding=(0,padding,0), bias=False, groups=groups),
...
class UNet3DGroupDilation4Branch(Module):
    def __init__(self, num_channels=3, num_classes=1, feat_channels=[32,64,128,256], residual=None, 
        ...
        # 编码器：卷积块
        self.enc_conv1 = Conv3D_Block_Group_Dilation(num_channels, feat_channels[0], residual=residual, groups=groups)
        self.enc_conv2 = Conv3D_Block_Group_Dilation(feat_channels[0], feat_channels[1], residual=residual, groups=groups)
        self.enc_conv3 = Conv3D_Block_Group_Dilation(feat_channels[1], feat_channels[2], residual=residual, groups=groups)
        
        if len(self.feat_channels) >= 4:
            self.enc_conv4 = Conv3D_Block_Group_Dilation(feat_channels[2], feat_channels[3], residual=residual, groups=groups) # 瓶颈层
...
if __name__ == '__main__':
    ...
    for cfg in configs:
        model = UNet3DGroupDilation4Branch(
            num_channels=3,
