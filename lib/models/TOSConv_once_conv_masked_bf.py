...
class TOSConv_once_conv_masked(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(
            torch.empty(out_channels, self.in_channels_per_group, temporal_size)
        )
        if bias:
        ...
        # Zero logits become a uniform distribution; after masking the diagonal,
        # this initializes each frame's background as the mean of the other frames.
    ...
    def forward(self, x):
        ...
        masked_weights = base_weights.unsqueeze(2) * self.temporal_exclusion_mask.unsqueeze(0).unsqueeze(0)
        denom = masked_weights.sum(dim=3, keepdim=True).clamp_min(torch.finfo(masked_weights.dtype).eps)
