...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument('--discribe', default='5帧不下采样') # 修改1
        self.parser.add_argument('--task', default='ctdet_points',
                                 help='task name.  ctdet_points |  ctdet ')
        self.parser.add_argument('--exp_name', default='I2PSOD', # 'unsupervised_iterative_layers_3_', # I2PSOD # # 修改2
                                 help='name of the experiments.')
        self.parser.add_argument('--layers', type=float, default=3, # 默认是3
                                 help='use decomp model or not.') 
        self.parser.add_argument('--model_name', default='I2PSOD', # sp_centerDet_minus # LightweightUnet3DDynamic # I2PSOD # 修改3
                                 help='name of the model.')
        self.parser.add_argument('--load_model', default='./weights/rs_car_new_multi/I2PSOD/I2PSOD_xrsy_supMode_0_seglen8_weights2025_12_23_20_24_37/model_best_dis_f1_best.pth',
        ...
        # train
        self.parser.add_argument('--lr', type=float, default=1.5e-3 /8 *8, # 1.25e-4,HiEUM with bs=6 # 1.5e-4 /8 *12 # 修改4
                                 help='learning rate for batch size 4.')
        self.parser.add_argument('--lr_step', type=str, default='10,30,45', #30,45
                                 help='drop learning rate by 10.')
        self.parser.add_argument('--num_epochs', type=int, default=55,  #55
                                 help='total training epochs.')
        self.parser.add_argument('--batch_size', type=int, default=1, # 6 # 修改5
                                 help='batch size')
        self.parser.add_argument('--val_intervals', type=int, default=5,
                                 help='number of epochs to run validation.')
        self.parser.add_argument('--seqLen', type=int, default=5, # 修改6
        ...
        self.parser.add_argument('--metric', type=int, default={'inference':True, # 修改 7
                                                                ...
                                                                'save_json':False, 'save_mat':True, 'f1_mode':['iou','dis']},
                                 help='how to test')
        ...
        self.parser.add_argument('--data_mode', type=str, default='multi',
                                 help='dataset name.')
        self.parser.add_argument('--datasetname', type=str, default='rs_car_new', # rs_car # aircraft # rs_car_new # sdm_car # 修改8
                                 help='dataset name.')
        self.parser.add_argument('--data_dir', type=str, default=r'E:/NUDT-Master/Academic/DataSet/RsCarData_New_Part/',  #/root/autodl-tmp/RsCarData/', # /root/autodl-tmp/AircraftDataset23/
                                 # 注意以 / 结尾
        ...
        self.parser.add_argument('--data_sampling', type=int, default=5, # 初始为1，表示使用全部数据；改为20表示每20帧采样1帧 
                                 help='data_sampling.')
        self.parser.add_argument('--multi2multi', type=bool, default=True, # 初始为1，表示使用全部数据；改为20表示每20帧采样1帧
                                 help='multi inputs and multi outputs, this can impact the last patch for test.')
        ...
        self.parser.add_argument('--hm_large_heatmap_flag', type=bool, default=True, # 修改11
        ...
        # net1 define
        self.parser.add_argument('--feat_channels', type=list, default=[16,32,64,128], # 修改12
                                 help='unet upsample channels')
        self.parser.add_argument('--input_channels', type=int, default=1,
                                 help='how many channles feature input net2')
        self.parser.add_argument('--T_pooling', type=bool, default=False, # 修改 13
                                 help='is pooling t dim or not')
        self.parser.add_argument('--groups', type=int, default=2,
                                 help='net1 conv groups(must be feat_channels % == 0)')
