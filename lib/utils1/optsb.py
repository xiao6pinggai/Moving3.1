...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument('--discribe', default='[ds10_conv2d_d2_+consine+GDfusion_v9]317反比增强，alpha=0.5，随机数种子317,cosmatch设置dialtion=2，不填充0，I2PSOD 20帧 net1三层次下采样T保持，全部为标准卷积,net1损失修改为:hm半径为HW,sigma/3,kt=3,均包含瓶颈层,融合简化，减少中间层,net1outonly') # 修改1 # enc修改为二分支空洞时空解耦，空间保留dilation,CDC，时间2 3 dilation,去掉0sum为正常时域3*3*3卷积，取消保底branch的1*1，时间后bnrelu conv1*1*1,然后和空间先cat再1*1*1降维输出  增加了归一化以及snext与sprev相乘，算得分，输出使用biqkv且不做v交互，qk矩阵共享，
        self.parser.add_argument('--task', default='ctdet_points',
                                 help='task name.  ctdet_points |  ctdet ')
        self.parser.add_argument('--exp_name', default='xrsy_k3_ds10_conv2d211+consine+GDfusion3d_v9_seqlen20_UNet3DWithNormalConv3D_net1outonly', # 'unsupervised_iterative_layers_3_', # I2PSOD # # 修改2
        ...
        self.parser.add_argument('--seqLen', type=int, default=20, help='number of images for per sample. Currently supports 5.')
