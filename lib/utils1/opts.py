...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument('--discribe', default='v16new_001_w7_k3') # 修改1 # enc修改为二分支空洞时空解耦，空间保留dilation,CDC，时间2 3 dilation,去掉0sum为正常时域3*3*3卷积，取消保底branch的1*1，时间后bnrelu conv1*1*1,然后和空间先cat再1*1*1降维输出  增加了归一化以及snext与sprev相乘，算得分，输出使用biqkv且不做v交互，qk矩阵共享， # SGNet_Linear10_MFE_TOSConv_T11_newnet2_cosv10
        self.parser.add_argument('--task', default='ctdet_points',
                                 help='task name.  ctdet_points |  ctdet ')
        self.parser.add_argument('--exp_name', default='v16new_001_w7_k3', # 'unsupervised_iterative_layers_3_', # I2PSOD # # 修改2
        ...
        self.parser.add_argument('--bottle_enhancement', type=str, default='block', help='bottleneck enhancement: None removes the middle block, block keeps the original conv block, MFE names such as cosv16 use the corresponding MFE module')
