...
#     #####################################################################
    ...
    # [子调度器 A] Warmup 阶段 (前 5-10 个 Epoch)
    # 这里的 "LinearLR" 实际上可以用作 Warmup。
    # start_factor=1.0 表示从 opt.lr 开始，end_factor=1.0 表示结束也是 opt.lr。
    # 这样就实现了：在前 warmup_epochs 里，学习率恒定为 opt.lr (高位平台)。
    warmup_epochs = opt.warmup_epochs # 假设你需要 10 个 epoch 来让 Stage 1 收敛
    scheduler_1 = LinearLR(optimizer, start_factor=0.5, end_factor=1, total_iters=warmup_epochs)

    # [子调度器 B] 周期重启阶段 (剩下的 Epoch)
    # 这里的 T_0=10 是指进入这个阶段后的周期
    scheduler_2 = CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2, eta_min=opt.lr*1e-2)

    # 3. 串联调度器 (SequentialLR)
    # milestones=[warmup_epochs] 表示：
    # 0 ~ 20 epoch: 运行 scheduler_1 (恒定高 LR)
    # 20 ~ end epoch: 运行 scheduler_2 (周期重启)
    scheduler = SequentialLR(optimizer, schedulers=[scheduler_1, scheduler_2], milestones=[warmup_epochs])
    start_epoch = 0
