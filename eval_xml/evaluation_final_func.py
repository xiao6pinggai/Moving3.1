...
ROOT_DIR = "/root/autodl-tmp/TDCNet-main"
# 确保根目录在sys.path首位（覆盖默认的子目录）
if ROOT_DIR not in sys.path:
    sys.path.insert(0, ROOT_DIR)
from eval_xml.utils_eval import eval_metric
...
def eval_func_final(results_dir_tol, data_dir=None, data_name=None, conf_ths = None, write_flag = True, 
    ...
    if conf_ths is not None:
        ...
        conf_thresh_all =  [0.2,0.25,0.3] # 不太耗时
...
if __name__ == '__main__':
    class opts(object):
        def __init__(self):
            ...
            self.parser.add_argument('--datasetname', default='sdm_car') # sdm_car rs_car_new
        def parse(self, args=''):
            opt = self.parser.parse_args()
            return opt
    opt = opts().parse()
    # results_dir = [
    #     'weights/rs_car_new_multi/I2PSOD/womfe_supMode_0_seglen10_weights2026_03_30_13_05_31/results_model_best_dis_f1_best/', # 必须以/结尾
    # ]
    # data_dir = '/root/autodl-tmp/RsCarData_New_Part/images/test1024/'
    # data_name  = [2,3,5,6,8,9,10]

    results_dir = [
        './weights/sdm_car_multi/I2PSOD_test/xrsy_resume25_k5_s10_2d+consine+GD1D_v9_975_1234_supMode_0_seglen10_weights2026_03_08_16_40_25/results_model_best_dis_f1_best/',
    ]
    data_dir = '/root/autodl-tmp/SDM-Car-New/images/test/'
    data_name  = os.listdir(data_dir)
    
    eval_func_final(results_dir, data_dir, data_name,eval_mode_metric='dis', opt=opt, xmlname='xml1', conf_ths=[0.255,0.26])